...
#-----------------------------3.2(1) Experiment 3-------------------------#
#-------------------------------------------------------------------------#
#1. Given a specific c, solve for the stable point values under a set of xD and xC
helper = function(x){
  ...
  # Relative utilities
  betaD <- 1.5  # 1. The unreasonable part: betaD is 1.5 times, while d is 8 times l
  betaC <- 1.1
  ...
  Pd <- 0  # Accumulated net income for the involution group
  Pc <- 0  # Accumulated net income for the sit-up group
  Pl <- 0  # Accumulated net income for the lay-flat group
  
  for (Nd in 0:(N - 1)){ 
    for (Nc in 0:(N - 1 - Nd)){
      # Expected income from individual strategy choices
      Nl <- N - 1 - Nd - Nc
...
#2. Given a specific c, traverse the parameter range of xD and xC to solve for the stable star values
get_star_by_c = function(c){
  all <- data.frame()
...
#------------------------------------------------------------------------------#
#-----------------------------3.1 Experiment 3 (beta)--------------------------#
#------------------------------------------------------------------------------#
#3. Given a specific c, solve for the stable point values under a set of xD and xC
...
#4. Given a specific c, traverse the parameter range of xD and xC to solve for the stable star values
get_star_by_c_beta = function(c){
  all <- data.frame()
