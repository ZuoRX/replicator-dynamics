...
#-----------------------------3.1 Experiment 3------------------------#
#---------------------------------------------------------------------#
# 1. Given a value for d, solve for the stable point values under a set of xD and xC
helper = function(x){
  ...
  Pd <- 0 # Net cumulative benefit of the involution group
  Pc <- 0 # Net cumulative benefit of the sit-up group
  Pl <- 0 # Net cumulative benefit of the lying-flat group
  for (Nd in 0:(N - 1)){ 
    for (Nc in 0:(N - 1 - Nd)){
...
# 2. Given a value for d, iterate through the parameter range of xD\xC to solve for stable star values
get_star_by_d = function(d){
  all = data.frame()
...
#------------------------------------------------------------------------------#
# 3. Given a value for d, solve for the stable point values under a set of xD and xC (with beta)
...
# 4. Given a value for d, iterate through the parameter range of xD\xC to solve for stable star values (with beta)
get_star_by_d_beta = function(d){
  all = data.frame()
