...
# Visualization of the first chart: xD & xC
df1 <- df %>% 
...
p1 <- df1 %>% 
  ...
  mutate(t1 = c(rep(df1$t[order(df1$t, decreasing = TRUE)], 3)))
...
# Visualization of the second chart: xD & xL
df1 <- df %>% 
  subset(xDstar >= 0 & xCstar == 0 & xLstar >= 0) %>% 
  data.table() %>% 
  .[, n := seq(.N), by = t] %>% 
...
#------------------------------------------------------------------------#
#-------------------------Experiment 4 Visualization (beta)--------------#
#------------------------------------------------------------------------#
...
# Visualization of the first chart: xD & xC (beta)
df1 <- df %>% 
  subset(xDstar >= 0 & xCstar >= 0 & xLstar == 0) %>% 
  data.table() %>% 
  .[, n := seq(.N), by = t] %>% 
...
# Visualization of the second chart: xD & xL (beta)
df1 <- df %>% 
  subset(xDstar >= 0 & xCstar == 0 & xLstar >= 0) %>% 
  data.table() %>% 
  .[, n := seq(.N), by = t] %>% 
...
# Visualization of the third chart: xC & xL (beta)
df1 <- df %>% 
  subset(xDstar == 0 & xCstar >= 0 & xLstar >= 0) %>% 
  data.table() %>% 
  .[, n := seq(.N), by = t] %>% 
