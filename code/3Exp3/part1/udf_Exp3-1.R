...
#2.给定一个d，遍历xD\xC的参数范围，求解稳定的star值
get_star_by_d = function(d){
  all =data.frame()
  for (x in seq(0.01,0.99,0.01)){
    for (y in seq(0.01,round(1-x,2),0.01)){
...
#4.给定一个d，遍历xD\xC的参数范围，求解稳定的star值
get_star_by_d_beta = function(d){
  all =data.frame()
  
  for (x in seq(0.02,0.98,0.02)){
    for (y in seq(0.02,round(1-x,2),0.02)){
